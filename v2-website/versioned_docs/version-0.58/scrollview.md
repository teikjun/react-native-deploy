...
`ScrollView` renders all its react child components at once. That can help with usability.
...
This is where `FlatList` comes into play. `FlatList` renders items lazily, only when they are about to appear, and removes items that scroll way off screen to save memory and processing time.
